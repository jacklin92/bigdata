...
(1)聲量分析: 根據以下資料，幫我撰寫一份至少200字的詳細的專業分析報告
以下是熱門程度，有多篇新聞報導提到:\n\n{key_occurrence_cat}\n\n
...
'''
    ...
    try:
        ...
        try:
            ...
            print("❌ JSON parse error. Raw response:", response.text)
            return JsonResponse({'error': '回應格式錯誤，請確認遠端服務是否正常'}, status=500)

    except requests.exceptions.Timeout as e:
        print("⏰ Timeout error:", e)
        return JsonResponse({'error': '連線逾時，請稍後再試'}, status=504)

    except requests.exceptions.RequestException as e:
        print("🌐 Request error:", e)
        return JsonResponse({'error': f'連線錯誤：{str(e)}'}, status=502)

    except Exception as e:
        print("🔥 Unexpected error:", e)
        return JsonResponse({'error': '產生報告時發生未知錯誤'}, status=500)

    # 回傳 AI 生成的報告
    return JsonResponse({'report': result['response']})

@csrf_exempt
def api_get_tts(request):
    if request.method == "POST":
        text = request.POST.get("text")
        lang = request.POST.get("lang", "zh-tw")  
        ...
        try:
            ...
            # 你可以選擇直接回傳 mp3 file
            return FileResponse(open(tmp_file.name, "rb"), content_type="audio/mpeg")
